...
def room(request, pk):
    ...
    room_messages = room.message_set.all() # get all messages from the room, query child objs of a specific room. message is the model name. one to many relationship
...
def update_user(request):
    ...
    if request.method == 'POST':
        form = UserForm(request.POST, instance=user) # pre-populate the form with the existing data
        if form.is_valid():
            form.save()
